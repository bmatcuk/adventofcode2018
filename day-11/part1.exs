defmodule AoC.Day11.Part1 do
  ...
  def run(serial_number) do
    acc = [build_row(299, serial_number), build_row(300, serial_number)]
    Enum.reduce(298..1, {"", 0, acc}, fn y, {result, largest, rows} ->
      rows = [build_row(y, serial_number) | Enum.take(rows, 2)]
      {sum, maxx} =
        Stream.zip(rows)
...
IO.puts AoC.Day11.Part1.run 7347
