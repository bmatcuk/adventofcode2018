defmodule AoC.Day6.Part2 do
  ...
  defp find_most_isolated_area({minx, miny, maxx, maxy, points}) do
    width = maxx - minx + 1
    height = maxy - miny + 1
    Enum.reduce(points, :array.new(width * height, {:default, 0}), fn {x, y}, acc ->
      Stream.flat_map(miny..maxy, fn j ->
        Stream.map(minx..maxx, fn i ->
          {(j - miny) * width + (i - minx), abs(i - x) + abs(j - y)}
        end)
      end)
      |> Enum.reduce(acc, fn {aryidx, mdist}, point_map ->
        :array.set(aryidx, :array.get(aryidx, point_map) + mdist, point_map)
      end)
    end)
    |> :array.to_list
    |> Enum.reduce(0, &(if(&1 < 10000, do: &2 + 1, else: &2)))
  end
end

IO.puts AoC.Day6.Part2.run 'input.txt'
