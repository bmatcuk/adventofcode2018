defmodule AoC.Day2.Part1 do
  ...
  def run(filename) do
    {has_two, has_three} =
      ...
      |> Stream.map(fn box_id ->
        String.to_charlist(box_id)
        |> Enum.reduce(%{}, fn chr, acc -> Map.update(acc, chr, 1, &(&1 + 1)) end)
      end)
      |> Enum.reduce({0, 0}, fn counts, {has_two, has_three} ->
        {
          if(Enum.any?(counts, fn {_, cnt} -> cnt == 2 end), do: has_two + 1, else: has_two),
          if(Enum.any?(counts, fn {_, cnt} -> cnt == 3 end), do: has_three + 1, else: has_three)
        }
      end)
    has_two * has_three
  rescue
...
IO.puts AoC.Day2.Part1.run 'input.txt'
