defmodule AoC.Day3.Part1 do
  ...
  def run(filename) do
    ...
    elem(
      ...
      |> Enum.reduce({0, :array.new(1_000 * 1_000)}, fn claim, acc ->
        Stream.flat_map(claim.y..(claim.y + claim.height - 1), fn j ->
          Stream.map(claim.x..(claim.x + claim.width - 1), &(&1 + j * 1_000))
        end)
        |> Enum.reduce(acc, fn i, {cnt, fabric} ->
          case :array.get(i, fabric) do
            :undefined -> {cnt, :array.set(i, :claimed, fabric)}
            :claimed -> {cnt + 1, :array.set(i, :overlap, fabric)}
            _ -> {cnt, fabric}
          end
        end)
      end),
...
IO.puts AoC.Day3.Part1.run 'input.txt'
